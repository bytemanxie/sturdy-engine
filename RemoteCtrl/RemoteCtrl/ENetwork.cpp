...
{
	if (m_params.m_type == ETYPE::ETypeUDP)
	{
		return threadUDPFunc();
	}
	else
	{
		return thraedTCPFunc();
	}
...
int EServer::threadUDPFunc()
{
	...
	while (!m_stop)
	{
		...
		if (ret > 0)
		{
			...
			//	printf("%s (%d): %s ip %s port %d \r\n", __FILE__, __LINE__, __FUNCTION__, client.GetIP().c_str(), ntohs(client.GetPort()));
			...
			//	buf.Update((void*)&lstclients.front(), lstclients.size());
			//	ret = sock->sendto(buf, client);
			//	printf("%s (%d): %s \r\n", __FILE__, __LINE__, __FUNCTION__);
			//}
		}
